...
export async function headsetRoutes(app: FastifyInstance) {
  ...
  app.post('/bulk', { preHandler: [auth, hasPermission('canManageHeadsets')] }, (request, reply) => 
    headsetController.createBulk(request, reply)
  );

  app.post('/', { preHandler: [auth, hasPermission('canManageHeadsets')] }, (request, reply) => 
    headsetController.create(request, reply)
  );

  app.put('/:id', { preHandler: [auth, hasPermission('canManageHeadsets')] }, (request, reply) => 
    headsetController.update(request, reply)
  );

  app.patch('/bulk', { preHandler: [auth, hasPermission('canManageHeadsets')] }, (request, reply) => 
    headsetController.updateBulk(request, reply)
  );

  app.delete('/:id', { preHandler: [auth, hasPermission('canDeleteHeadsets')] }, (request, reply) => 
