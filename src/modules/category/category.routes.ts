...
export async function categoryRoutes(app: FastifyInstance) {
  ...
  app.post('/', { preHandler: [hasPermission('canManageCategories')] }, (request, reply) => 
    categoryController.create(request, reply)
  );

  app.get('/', (request, reply) => categoryController.list(request, reply));
  
  app.put('/:id', { preHandler: [hasPermission('canManageCategories')] }, (request, reply) => 
    categoryController.update(request, reply)
  );
  
  app.delete('/:id', { preHandler: [hasPermission('canDeleteItems')] }, (request, reply) => 
